...
class TestDeceasedPatientsAPI:
    """Тесты для API умерших пациентов."""

    @pytest.mark.asyncio
    async def test_get_deceased_patients_success(self, client, mock_deceased_patients_service):
        """Проверка успешного получения данных умерших пациентов."""
        response_template = {
        ...
        mock_deceased_patients_service.get_data.return_value = DeceasedResponse(**response_template)

        response = client.get(
            "/api/journal/deceased-patients",
            params={
        ...
        called_filter = mock_deceased_patients_service.get_data.call_args[0][0]
        assert isinstance(called_filter, DeceasedFilter)
        assert called_filter.date_from == "2025-01-01"
        assert called_filter.date_to == "2025-12-31"

    @pytest.mark.asyncio
    async def test_get_deceased_patients_with_filters(self, client, mock_deceased_patients_service):
        """Проверка получения данных умерших пациентов с фильтрами."""
        response_template = {
            "hospital_name": "Коммунальное государственное предприятие на праве хозяйственного ведения \"Областная многопрофильная больница города Жезказган\" управления здравоохранения области Улытау",
            "items": [],
            "total": 0,
    ...
    @pytest.mark.asyncio
    async def test_get_deceased_patients_missing_required_params(self, client):
        """Проверка получения ошибки при отсутствии обязательных параметров."""
        response = client.get("/api/journal/deceased-patients")
