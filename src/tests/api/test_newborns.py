...
class TestNewbornsAPI:
    """Тесты для API новорожденных."""

    @pytest.mark.asyncio
    async def test_get_newborns_success(self, client, mock_newborns_service):
        """Проверка успешного получения данных новорожденных."""
        response_template = {
        ...
        mock_newborns_service.get_data.return_value = NewbornResponse(**response_template)

        response = client.get(
            "/api/journal/newborns",
            params={
        ...
        called_filter = mock_newborns_service.get_data.call_args[0][0]
        assert isinstance(called_filter, NewbornFilter)
        assert called_filter.date_from == "2025-01-01"
        assert called_filter.date_to == "2025-12-31"
        assert called_filter.status == NewbornStatus.ALL

    @pytest.mark.asyncio
    async def test_get_newborns_with_filters(self, client, mock_newborns_service):
        """Проверка получения данных новорожденных с фильтрами."""
        response_template = {
            "hospital_name": "Коммунальное государственное предприятие на праве хозяйственного ведения \"Областная многопрофильная больница города Жезказган\" управления здравоохранения области Улытау",
            "items": [],
            "total": 0,
