...
class TestMaternityAssetsAPI:
    """Тесты для API активов роддома."""

    @pytest.mark.asyncio
    async def test_get_maternity_assets_success(self, client, mock_maternity_assets_service):
        """Проверка успешного получения активов роддома."""
        response_template = {
        ...
        mock_maternity_assets_service.get_data.return_value = MaternityResponse(**response_template)

        response = client.get(
            "/api/journal/maternity-assets",
            params={
        ...
        called_filter = mock_maternity_assets_service.get_data.call_args[0][0]
        assert isinstance(called_filter, MaternityFilter)
        assert called_filter.date_from == "2025-01-01"
        assert called_filter.date_to == "2025-12-31"
        assert called_filter.status == MaternityStatus.ALL

    @pytest.mark.asyncio
    async def test_get_maternity_assets_with_filters(self, client, mock_maternity_assets_service):
        """Проверка получения активов роддома с фильтрами."""
        response_template = {
            "hospital_name": "Коммунальное государственное предприятие на праве хозяйственного ведения \"Областная многопрофильная больница города Жезказган\" управления здравоохранения области Улытау",
            "items": [],
            "total": 0,
        ...
        assert called_filter.status == MaternityStatus.ACTIVE
        assert called_filter.sort_by == "patient_name_asc"
        assert called_filter.page == 2
        assert called_filter.limit == 20

    @pytest.mark.asyncio
    async def test_get_maternity_assets_missing_required_params(self, client):
        """Проверка получения ошибки при отсутствии обязательных параметров."""
        response = client.get("/api/journal/maternity-assets")
